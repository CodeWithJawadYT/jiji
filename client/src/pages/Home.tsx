  ...
  return <div className="site-shell">
    ...
    <main>
      <section id="home" className="hero" style={{ backgroundImage: `linear-gradient(90deg, rgba(3,5,9,.92) 0%, rgba(3,5,9,.72) 44%, rgba(3,5,9,.16) 100%), url(${heroArt})` }}>
        <Suspense fallback={null}><JawyxWebGLHero /></Suspense><div className="hero-grid" aria-hidden="true" /><div className="hero-orbit" aria-hidden="true"><img src={companionMark} alt="" /></div>
        ...
        <div className="hero-meta"><span>EST. / 2024</span><span>TORONTO · WORLDWIDE</span><span>01 <span className="muted">/ 06</span></span></div><button className="scroll-cue" onClick={() => jump("about")}><span>Scroll to explore</span><ChevronDown size={17} /></button>
      </section>

      <section id="about" className="manifesto section-dark"><div className="section-rail"><span>02</span><span>Manifesto</span></div><div className="manifesto-inner"><p className="eyebrow">The JawyXDevs approach</p><h2>We don't just build websites.<br /><span>We build digital experiences.</span></h2><div className="manifesto-bottom"><p className="large-copy">The web is where your brand proves what it is. We combine modern frontend engineering, intentional interaction, and search-ready architecture to make that proof impossible to miss.</p><div className="capability-list"><span>High-performance development</span><span>Responsive interfaces</span><span>Interactive 3D experiences</span><span>Conversion-focused UX</span></div></div></div></section>

      <section id="services" className="services section-dark"><div className="section-rail"><span>03</span><span>Capabilities</span></div><div className="services-inner"><div className="section-heading"><p className="eyebrow">What we engineer</p><h2>Built for the<br /><span>next interaction.</span></h2><p>From the first line of code to the last pixel on screen, every layer has a job.</p></div><div className="service-list">{services.map(([number, title, desc]) => <article className="service-row" key={number}><span className="service-number">{number}</span><h3>{title}</h3><p>{desc}</p><ArrowUpRight className="service-arrow" size={21} /></article>)}</div></div></section>

      <section id="work" className="work-section"><div className="section-rail"><span>04</span><span>Selected work</span></div><div className="work-inner"><div className="section-heading work-heading"><p className="eyebrow">A selection of live work</p><h2>Work with<br /><span>signal.</span></h2><p>Real projects. Real links. No invented numbers.</p></div><div className="featured-project"><a href={projects[activeProject].url} target="_blank" rel="noreferrer" className="project-stage"><img src={projects[activeProject].art} alt={`${projects[activeProject].name} abstract project artwork`} /><div className="stage-overlay" /><span className="stage-index">{projects[activeProject].number} / 06</span><span className="visit-pill">Visit website <ArrowUpRight size={15} /></span><div className="project-title"><p>{projects[activeProject].category}</p><h3>{projects[activeProject].name}</h3></div></a><div className="project-caption"><p>{projects[activeProject].note}</p><span>Open live site ↗</span></div></div><div className="project-selector">{projects.map((project, i) => <button key={project.name} className={activeProject === i ? "active" : ""} onClick={() => setActiveProject(i)}><span>{project.number}</span><strong>{project.name}</strong><small>{project.category}</small><ArrowUpRight size={16} /></button>)}</div></div></section>

      <section id="pricing" className="pricing section-dark"><div className="section-rail"><span>05</span><span>Engagement</span></div><div className="pricing-inner"><div className="section-heading"><p className="eyebrow">Choose your altitude</p><h2>Clear scope.<br /><span>Strong signal.</span></h2><p>Simple packages for shipping a sharper digital presence.</p></div><div className="price-grid">{plans.map(plan => <article className={`price-card ${plan.featured ? "featured" : ""}`} key={plan.name}>{plan.featured && <div className="recommended">Recommended</div>}<p className="plan-label">{plan.name}</p><div className="price">{plan.price}</div><p className="plan-description">{plan.description}</p><div className="feature-stack">{plan.features.map(feature => <span key={feature}><i />{feature}</span>)}</div><button className="price-button" onClick={() => jump("contact")}>Choose {plan.name} <ArrowUpRight size={15} /></button></article>)}</div><p className="custom-note">Need something custom? <button onClick={() => jump("contact")}>Let's build it <ArrowUpRight size={14} /></button></p></div></section>

      <section className="principles"><div className="principles-orb" aria-hidden="true" /><div className="principles-inner"><p className="eyebrow">Why JawyXDevs</p><h2>The standard is<br /><span>how it feels.</span></h2><div className="principle-grid"><div><span>01</span><h3>Performance first</h3><p>Fast, optimized experiences that respect attention.</p></div><div><span>02</span><h3>Built for every screen</h3><p>Responsive from the first breakpoint to the widest display.</p></div><div><span>03</span><h3>Search ready</h3><p>SEO, AEO, and GEO-aware architecture from day one.</p></div><div><span>04</span><h3>Interactive by design</h3><p>Motion that communicates, not distracts.</p></div></div></div></section>

      <section id="contact" className="contact-section"><div className="contact-inner"><div><p className="eyebrow">06 / Start a project</p><h2>Make the first<br /><em>screen</em> do more.</h2></div><div className="contact-side"><p>Tell us what you’re building, what needs to move, and where you want to go next.</p><a href="mailto:hello@jawyxdevs.com" className="contact-link">hello@jawyxdevs.com <ArrowUpRight size={20} /></a><p className="contact-note">Available for select projects worldwide.</p></div></div></section>
    </main>

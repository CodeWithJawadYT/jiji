  ...
  return <div className="site-shell">
    ...
    <main>
      <section id="home" className="hero" style={{ backgroundColor: "#000000" }}>
        <Suspense fallback={null}><JawyxWebGLHero /></Suspense><Antigravity count={190} magnetRadius={6} ringRadius={7} waveSpeed={0.4} waveAmplitude={1} particleSize={0.9} lerpSpeed={0.05} color="#6f8fa6" autoAnimate particleVariance={0.85} rotationSpeed={0.03} depthFactor={1} pulseSpeed={3} particleShape="capsule" fieldStrength={10} /><PixelTrail gridSize={46} trailSize={0.11} maxAge={340} interpolate={5} color="#dff6ff" />
      </section>
      ...
      <section id="work" className="work-section"><div className="section-rail"><span>04</span><span>Selected work</span></div><div className="work-inner"><div className="section-heading work-heading"><p className="eyebrow">A selection of live work</p><h2>Work with<br /><span>signal.</span></h2><p>Real projects. Real links. No invented numbers.</p></div><div className="featured-project reveal-on-scroll"><a ref={projectStageRef} href={projects[activeProject].url} target="_blank" rel="noreferrer" className="project-stage"><img src={projects[activeProject].art} alt={`${projects[activeProject].name} abstract project artwork`} /><div className="stage-overlay" /><span className="stage-index">{projects[activeProject].number} / 06</span><span className="visit-pill">Visit website <ArrowUpRight size={15} /></span><div className="project-title"><p>{projects[activeProject].category}</p><h3>{projects[activeProject].name}</h3></div></a><div className="project-caption"><p>{projects[activeProject].note}</p><span>Open live site ↗</span></div></div><div className="project-selector">{projects.map((project, i) => <button key={project.name} className={activeProject === i ? "active" : ""} onClick={() => setActiveProject(i)}><span>{project.number}</span><strong>{project.name}</strong><small>{project.category}</small><ArrowUpRight size={16} /></button>)}</div></div></section>

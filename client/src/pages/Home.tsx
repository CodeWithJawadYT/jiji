...
export default function Home() {
  ...
  return <div className="site-shell">
    ...
    <main>
      ...
      <section id="work" className="work-section"><div className="section-rail"><span>04</span><span>Selected work</span></div><div className="work-inner"><div className="section-heading work-heading"><p className="eyebrow">A selection of live work</p><h2>Work with<br /><span>signal.</span></h2><p>Real projects. Real links. No invented numbers.</p></div><div className="featured-project reveal-on-scroll"><a ref={projectStageRef} href={projects[activeProject].url} target="_blank" rel="noreferrer" className="project-stage"><img src={projects[activeProject].art} alt={`${projects[activeProject].name} abstract project artwork`} /><div className="stage-overlay" /><span className="stage-index">{projects[activeProject].number} / 06</span><span className="visit-pill">Visit website <ArrowUpRight size={15} /></span><div className="project-title"><p>{projects[activeProject].category}</p><h3>{projects[activeProject].name}</h3></div></a><div className="project-caption"><p>{projects[activeProject].note}</p><span>Open live site ↗</span></div></div><div className="masonry-intro"><span>Scroll / explore the archive</span><span>Six live builds / open each tile</span></div><Masonry items={masonryItems} ease="power3.out" duration={0.7} stagger={0.07} animateFrom="bottom" scaleOnHover hoverScale={0.96} blurToFocus /></div></section>

...
export function JawyxWebGLHero() {
  ...
  useEffect(() => {
    ...
    addForm(new THREE.IcosahedronGeometry(22, 1), [145, 2, -205], [0.2, 0.2, 0], 0.78, true);
    addForm(new THREE.OctahedronGeometry(17, 0), [210, -44, -255], [0.4, 0.2, 0.8], 0.82, false);
    addForm(new THREE.TorusKnotGeometry(10, 2.6, 80, 12), [118, 66, -240], [0.4, 0.7, 0.25], 0.58, true);
    ...
    if (!reducedMotion) {
      refs.scrollTrigger = ScrollTrigger.create({ trigger: home, start: "top top", end: "bottom top", scrub: 1.2, onUpdate: (self) => { refs.camera.position.y = THREE.MathUtils.lerp(6, -18, self.progress); refs.camera.position.z = THREE.MathUtils.lerp(mobile ? 31 : 26, mobile ? 47 : 58, self.progress); refs.camera.rotation.z = self.progress * 0.035; } });
    }
    ...
    return () => { refs.disposed = true; cancelAnimationFrame(refs.raf); refs.scrollTrigger?.kill(); window.removeEventListener("pointermove", onPointer); window.removeEventListener("resize", resize); refs.stars.forEach((star) => { star.geometry.dispose(); (star.material as THREE.Material).dispose(); }); refs.forms.forEach((form) => { form.geometry.dispose(); (form.material as THREE.Material).dispose(); }); nebulaGeometry.dispose(); nebulaMaterial.dispose(); atmosphereGeometry.dispose(); atmosphereMaterial.dispose(); refs.renderer?.dispose(); refs.composer?.dispose(); };
  }, []);

  return <div ref={mountRef} className="jawyx-webgl" aria-hidden="true"><canvas ref={canvasRef} /></div>;
}

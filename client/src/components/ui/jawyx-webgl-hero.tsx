...
export function JawyxWebGLHero() {
  ...
  useEffect(() => {
    ...
    const createTerrain = () => {
      ...
      createDistantLayer(1.18, -320, 4, 0x4e6673, 0xb8d0dc); createDistantLayer(1.52, -760, 11, 0x334b59, 0x8caab9); createDistantLayer(1.92, -1250, 19, 0x203743, 0x5f7d8c); const alpineBackdrop = new THREE.Mesh(new THREE.PlaneGeometry(16000, 9000, 1, 1), new THREE.MeshBasicMaterial({ map: new THREE.TextureLoader().load("/manus-storage/portfolio-exact_3d139745.jpg", texture => { texture.colorSpace = THREE.SRGBColorSpace; }), transparent: true, opacity: .82, depthWrite: false, depthTest: false, fog: true })); alpineBackdrop.position.set(0, 260, -2350); refs.scene.add(alpineBackdrop); refs.terrain.push(alpineBackdrop);
    };

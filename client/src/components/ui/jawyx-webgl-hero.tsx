...
export function JawyxWebGLHero() {
  ...
  useEffect(() => {
    ...
    // Background visuals intentionally removed: the hero remains a pure black field while the existing camera/scroll architecture stays active.
  ...
  const goTo = (id: "work" | "contact") => { const target = document.getElementById(id); if (!target) return; window.history.pushState({}, "", `#${id}`); const top = Math.max(0, target.getBoundingClientRect().top + window.scrollY - 72); window.scrollTo({ top, behavior: "smooth" }); };
  return <div ref={mountRef} className="mountain-journey"><canvas ref={canvasRef} className="mountain-canvas" aria-hidden="true" /><div className="mountain-side" style={{ opacity: Math.max(0, 1 - progress * 14) }}><img src={logo} alt="JawyXDevs logo" /><span>JAWYXDEVS // DIGITAL SYSTEMS</span></div><div className="mountain-copy" style={{ opacity: Math.max(0, 1 - progress * 14) }}><p className="mountain-kicker">ELITE WEB ENGINEERING STUDIO</p><h1 ref={titleRef}>{title.split(" ").map((word, index) => <span className={`mountain-title-word mountain-word-${index}`} key={`${word}-${index}`}>{word.split("").map((char, charIndex) => <span className="mountain-title-char" key={`${char}-${charIndex}`}>{char}</span>)}<span className="mountain-title-space">&nbsp;</span></span>)}</h1><div ref={subtitleRef} className="mountain-subtitle"><p className="mountain-subtitle-line">Stand at the summit of a digital world built to move.</p><p className="mountain-subtitle-line">Scroll to descend through the JawyXDevs experience.</p></div><div className="mountain-actions"><button type="button" className="button button-primary" onClick={() => goTo("work")}>VIEW OUR WORK <ArrowUpRight size={16} /></button><button type="button" className="button button-ghost" onClick={() => goTo("contact")}>START A PROJECT <ArrowUpRight size={16} /></button></div></div><div ref={progressRef} className="mountain-progress"><span>DESCEND</span><i><b style={{ width: `${progress * 100}%` }} /></i><strong>{String(section).padStart(2, "0")} / 05</strong><ChevronDown size={14} /></div><div className="mountain-meta">SUMMIT / 00 <span>TORONTO · WORLDWIDE</span></div></div>;
}
